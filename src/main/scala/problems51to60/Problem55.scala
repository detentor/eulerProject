...
object Problem55 
{
	...
	{
		def isLychrel(x : BigInt, numIte : Int = 0) : Boolean = if (x.toString == x.toString.reverse && numIte > 0) false 
											  					else if (numIte > 49) true
											  					else isLychrel(x + BigInt(x.toString.reverse), numIte + 1)

		println((1 to 10000).filter(x => isLychrel(x)).size)
	}
}
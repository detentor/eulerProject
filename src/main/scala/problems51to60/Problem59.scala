...
//and the encryption key in different locations, 
...
//the key is repeated cyclically throughout the message. The balance for this method is using a 
...
object Problem59 
{
	def main(args: Array[String]) 
	{
		...
		val text = data.replaceAll(",", " ").split(" ").map(_.toInt.toChar)
		
		val tries = for (a <- 96 until 123; b <- 96 until 123; c <- 96 until 123) 
						yield a.toChar.toString + b.toChar.toString + c.toChar.toString

		def encrypt(text : Array[Char], key : Array[Char]) = for (i <- 0 until text.length; val pos = i % key.length)
																yield (text(i).toInt ^ key(pos)).toChar
		
		def isValid(theChar : Char) = theChar == 32 || (theChar > 64 && theChar < 123)
		def isFinished(text : Array[Char]) = text.count(isValid) / text.length.toDouble > 0.95d
		
		val passcode = tries.find(key => isFinished(encrypt(text, key.toCharArray).toArray)).get
		val decrypted = encrypt(text, passcode.toCharArray).mkString
		
		println(passcode)
		println(decrypted)
		println(decrypted.map(_.toInt).sum)
	}
}